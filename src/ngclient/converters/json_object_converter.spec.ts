...
describe('JSONObjectConverter', () => {
    ...
    beforeEach(() => {
        ...
        specTypesService.registerType('Tab', Tab);
        specTypesService.registerType('TabHolder', TabHolder);
    });
    ...
    const createTabObjectFromServer = (): Tab => {
    ...
    it('object created and changed from client side', () => {
        const tab: Tab = createTabObjectFromServer();
        tab.name = 'test2';
    ...
    it('nested object has 2 props of the same type, both with values, then the sub-values change the property to which they are assigned', () => {
        ...
        expect(tabHolder.getStateHolder().allChanged).toBe(true, 'should have all changed as it is new');
